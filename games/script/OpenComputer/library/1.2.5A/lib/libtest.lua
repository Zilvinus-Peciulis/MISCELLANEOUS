...
lib.robot = function( go ) -- while not rb.forward() and	 gps.findWaypoints(100) do rb.swing() end return;
...
-----Waypoint
 ...
 local function printWaypoint () print (" pos x: " .. wX .." y:"..wY .. ",z:".. wZ);return true;end
  ...
  --Y go UP OR DOWN OFFSET
   if wY > offsetY then while  wY ~= offsetY do lib.robot("up") ;  wY=wY-1;printWaypoint (); end end
   if wY < offsetY  then while  wY ~= offsetY do lib.robot("down") ;    wY=wY+1;printWaypoint (); end end

 --X
   if wX > 0 then  lib.look ("E");  while  wX ~= 0 do lib.robot("forward") ;wX=wX-1; printWaypoint (); end end
   if wX < 0  then lib.look ("W"); while  wX ~= 0 do lib.robot("forward") ;  wX=wX+1; printWaypoint (); end end

 --Z
   if wZ > 0 then      lib.look ("S");  while  wZ ~= 0 do lib.robot("forward") ;  wZ=wZ-1;printWaypoint (); end end
   if  wZ < 0 then lib.look ("N");  while  wZ ~= 0 do lib.robot("forward") ;  wZ=wZ+1;printWaypoint (); end  end

   --Y
   if wY > 0 then while  wY ~= 0 do lib.robot("up") ;  wY=wY-1;printWaypoint (); end end
   if wY < 0  then while  wY ~= 0 do lib.robot("down") ;    wY=wY+1;printWaypoint (); end end
end
...
lib.goto  = function(x,y,z ,offsetY)
  assert (type(x)=="number" and type(y)=="number" and type(z)=="number", "-->error:x,y,z must be a  number" )
   ...
   local function pos () print ("x"..(x).."-rX" .. (rX) ..",y"..(y).."-rY:"..(rY) .. ",z"..(z).."-rZ:".. (rZ)); os.sleep(1); end
   --x=x-.5;y=y-.5;z=z-.5; --offset of coordinates
   offsetY = offsetY or rY;
   pos ();
 --  --Y go UP OR DOWN OFFSET
 --Y
    if rY < y then while  rY ~= y do lib.robot("up") ;  rY=rY+1;pos (); end end
    if rY > y  then while  rY ~= y do lib.robot("down") ;    rY=rY-1;pos (); end end
 -- --X
    if rX < x then     lib.look ("E");  while  rX ~= x do lib.robot("forward") ;rX=rX+1; pos (); end end
    if rX > x  then    lib.look ("W"); while  rX ~= x do lib.robot("forward") ;  rX=rX-1; pos (); end end
 --
  --Z
   if rZ < z then      lib.look ("S");  while  rZ ~= z do lib.robot("forward") ;  rZ=rZ+1;pos (); end end
   if rZ > z then      lib.look ("N");  while  rZ ~= z do lib.robot("forward") ;  rZ=rZ-1;pos (); end  end
